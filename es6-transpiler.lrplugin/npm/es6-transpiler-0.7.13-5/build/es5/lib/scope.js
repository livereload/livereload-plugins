...
Scope.prototype.hasFunctionScopeBetween = function(outer, withoutSelf) {
	var scope = this;
	if( withoutSelf ) {
		if( scope === outer ) {
			return false;
    ...
    throw new Error("wasn't inner scope of outer #1");
};

Scope.prototype.hasLoopScopeBetween = function(outer, withoutSelf) {
	var scope = this;
	if( withoutSelf ) {
		if( scope === outer ) {
			return false;
	...
	for ( ; scope; scope = scope.parent) {
		...
		if (isLoop(scope.node)) {
			return true;
		}
	}

	throw new Error("wasn't inner scope of outer #2");
};
